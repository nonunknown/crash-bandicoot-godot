...
func follow():
	...
	if elapsed < .5:
		global_transform.origin = lerp(global_transform.origin, global_transform.origin + Vector3.UP *150,0.2)
		return
	
	
	global_transform.origin = lerp(global_transform.origin,Vector3(cos(elapsed) + target.x, ( sin(elapsed) ) + target.y,target.z),0.1)
